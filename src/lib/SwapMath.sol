    ...
    function computeSwapStep(
        ...
        if (exactIn) {
            ...
            uint256 amountRemainingLessFee =
                FullMath.mulDiv(SafeCast.toUint256(amountRemaining), 1e6 - feePips, 1e6);
            ...
            // Calculate next sqrt ratio
            if (SafeCast.negToUint256(amountRemaining) >= amountOut) {
                sqrtRatioNextX96 = sqrtRatioTargetX96;
            } else {
                // 0 for 1 = target < next <= current
                // 1 for 0 = current <= next < target
                sqrtRatioNextX96 = SqrtPriceMath.getNextSqrtPriceFromOutput(
                    sqrtRatioCurrentX96, liquidity, SafeCast.negToUint256(amountRemaining), zeroForOne
                );
            }
        }
        ...
        if (!exactIn && amountOut > SafeCast.negToUint256(amountRemaining)) {
            amountOut = SafeCast.negToUint256(amountRemaining);
        }

        // Calculate fee on amount in
        if (exactIn && sqrtRatioNextX96 != sqrtRatioTargetX96) {
            // Take the remainder of the maximum input as fee
            feeAmount = SafeCast.toUint256(amountRemaining) - amountIn;
        } else {
